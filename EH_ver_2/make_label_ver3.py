...
for itr_1 in range(MAX_ITERATION):
    print("ver3_30timeslot_MAX_ITERATION: %d, Harvesting_efficiency: %f" % (MAX_ITERATION,par.Harvesting_efficiency))
    print("current time: %s, itr_1: %d" % (datetime.datetime.now(), itr_1))

    # make new start point of a realization
    # Only dL channel information
    h_UE_time = np.zeros(shape=(par.N_UE,par.N_time*10))
    ...
    for itr_2 in range(par.N_realization):
        ...
        for flow in range(h_UE_time.shape[1]/3):
            ...
            battery, delay, num_final_decision = find_the_best_decision_for_the_realization(h_UE_time[:,3*flow:3*flow+3],par, exist_ind,battery, delay)

            h_quantized = quantize_H(h_UE_time[:,3*flow:3*flow+3],par) # UE1_past, UE1_current, UE2_past, UE2_current, UE1_future, UE2_future
...
np.save('/home/mukim/Desktop/EH_ver_2/label/ver3_label_30timeslot_reali_5_itr_5000_eff_0_7_result_table.npy',result_table)
np.save('/home/mukim/Desktop/EH_ver_2/label/ver3_label_30timeslot_reali_5_itr_5000_eff_0_7.npy',real_label)
