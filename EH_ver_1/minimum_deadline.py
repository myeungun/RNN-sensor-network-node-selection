...
def main(argv=None):
    ...
    for itr in xrange(MAX_ITERATION):
        ...
        for train_count in xrange(train_data_size):
            ...
            deadline_info = np.concatenate((deadline.Expected_UL_deadline, deadline.True_DL_deadline), axis=0)
            selected_ind = np.argmin(deadline_info)

            nd_array_selection = np.zeros(shape=[1,2*sys_param.N_USER])
            nd_array_selection[0][selected_ind]=1
        ...
        np.save('/home/mukim/Desktop/EH/minimum_deadline_penalty_count.npy', save_penalty_count)
